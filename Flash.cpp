...
namespace Cosmic
{
    ...
    void Flash::Show(float dt)
    {
        if (_flashOn)
        {
            float alpha = (float)_shape.getFillColor().a + (FLASH_SPEED * dt);
            ...
            _shape.setFillColor(sf::Color(255.0f, 255.0f, 255.0f, alpha));
        }
        else
        {
            float alpha = _shape.getFillColor().a - (FLASH_SPEED * dt);
